...
final dashboardProvider = FutureProvider<void>((ref) async {
  final currentMonth =
      await TransactionMethods().currentMonthDailyTransactions();
  final lastMonth = await TransactionMethods().lastMonthDailyTransactions();

  ref.read(incomeProvider.notifier).state = currentMonth.fold(
      0, (previousValue, element) => previousValue + element['income']);
  ref.read(expenseProvider.notifier).state = currentMonth.fold(
